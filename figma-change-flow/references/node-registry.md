...
### 1-2. 화면별 SoT 예시 노드 (네이티브 리스트 형식 적용 완료 — 신규 작성 시 clone)

> 2026-06-17 통일: 아래 노드는 전부 네이티브 리스트 형식(폰트 버그 회피) 적용 완료. 같은 기능군 신규 화면은 해당 화면을 clone해서 시작한다. 과거 `· `/`  - ` 수동 접두 방식은 폰트 버그 유발 반례 → 쓰지 말 것.
...
| 알람 | 상세 | `[node-id]` | `CONT-09_10_04` (알람 설정 > 상세, x=26111로 이동됨) | 검증 2026-06-19 ✓ |
